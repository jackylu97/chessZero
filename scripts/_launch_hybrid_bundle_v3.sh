#!/bin/bash
# v3 = bundle MINUS symmetry augmentation (root cause of v1/v2 reward-precision damage
# via the shared dynamics trunk — see strategy doc §11-12). Five untainted mechanisms kept.
...
#   (symmetry-augment REMOVED in v3 — reward-precision root cause, doc §11-12)
...
RUN=2026_07_05_hybrid_bundle_v3
...
tmux new-session -d -s selfplay "PYTHONPATH=. .venv/bin/python -u scripts/_faulthandler_bootstrap.py scripts/train.py \
  ...
  --tb-rollout-fill \
  2>&1 | tee /workspace/chessZero/selfplay_hybrid_bundle_v3.log"
