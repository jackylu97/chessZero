#!/bin/bash
# 2026_07_04_hybrid_bundle: the FROZEN next-run bundle on the validated base
...
tmux new-session -d -s selfplay "PYTHONPATH=. .venv/bin/python -u scripts/_faulthandler_bootstrap.py scripts/train.py \
  ...
  --tb-rollout-fill \
  2>&1 | tee /workspace/chessZero/selfplay_hybrid_bundle.log"
