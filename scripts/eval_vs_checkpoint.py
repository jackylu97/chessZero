...
def load_network(ckpt_path, game, cfg, device):
    ...
    net = MuZeroNetwork(
        ...
        # XL/attention arch (from the config preset):
        value_head_planes=getattr(cfg, "value_head_planes", 1),
        value_head_blocks=getattr(cfg, "value_head_blocks", 0),
        ...
        use_pred_attention=getattr(cfg, "use_pred_attention", False),
        use_smolgen=getattr(cfg, "use_smolgen", True),
        attn_layers=getattr(cfg, "attn_layers", 4),
        attn_heads=getattr(cfg, "attn_heads", 4),
        pred_attn_layers=getattr(cfg, "pred_attn_layers", 2),
        hybrid_stem_blocks=getattr(cfg, "hybrid_stem_blocks", 0),
    )
